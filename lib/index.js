...
class Page {
  ...
  static async open(remote_url, options = {}) {
    ...
    let browser    = await retryUntil(async () => {
      try {
    ...
    if(!browser)
      throw new Error('Cannot reach Browser');
    ...
    let page       = await retryUntil(async () => {
      try {
        ...
        if(!page._client.eventsMap.get('Page.lifecycleEvent')[0].length)
          return false;
    ...
    if(!page)
      throw new Error('Cannot get active page');
